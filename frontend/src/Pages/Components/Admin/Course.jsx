...
const Course = () => {
  ...
  useEffect(() => {
    const fetchCourseRequests = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/admin/${data}/approve/course`);
  ...
  const handleAccept = async (id, info) => {
    ...
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/admin/${data}/approve/course/${id}`, {
        Isapproved: true,
        email: info.Email,
        Firstname: info.enrolledteacher,
      }, {
